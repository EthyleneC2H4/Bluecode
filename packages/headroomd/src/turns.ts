...
export function isCompactionReplacement(message: ChatMessage): boolean {
  return (
    ...
    message.info.id === `compaction-${message.archive.historyHash}`
  )
}
...
export function legacyMessageHashInput(message: ChatMessage): {
  id: string
  role: string
  textParts: string
...
export async function legacyContentHash(message: ChatMessage): Promise<string> {
  return sha256Hex(canonicalJSON(legacyMessageHashInput(message)))
}

/** Hash every persisted field in order; never collapse part boundaries or statuses. */
export function messageHashInput(message: ChatMessage): unknown {
  return {
    version: 2,
    info: { id: message.info.id, role: message.info.role },
    parts: message.parts,
    ...(message.archive ? { archive: message.archive } : {}),
    ...(message.protected ? { protected: true } : {}),
  }
}
export function contentDigest(message: ChatMessage): string {
  return createHash("sha256")
    .update(canonicalJSON(messageHashInput(message)))
    .digest("hex")
}
export async function contentHash(message: ChatMessage): Promise<string> {
  return contentDigest(message)
}
