 ...
 */
import { buildMemory, renderMemory } from "./memory"
...
export function historySummary(turns: Turn[]): string {
  return renderMemory(buildMemory(turns.flatMap((turn) => turn.messages)))
}
...
export function messageTokens(message: ChatMessage): number {
  return estimateTokens(messageExcerpt(message))
}
