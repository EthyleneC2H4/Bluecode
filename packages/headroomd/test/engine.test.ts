...
function history(turnCount: number): ChatMessage[] {
  ...
  for (let i = 0; i < turnCount; i++) {
    messages.push(
      user(
        `u${i}`,
        `第 ${i} 轮：请分析模块 alpha 的行为。${"需要逐项核对输入、状态转换和异常边界。".repeat(8)}`
      )
    )
    messages.push(
      assistant(
        `a${i}`,
        `分析完成：模块 alpha 第 ${i} 轮结论。${"已核对输入、状态转换、恢复路径和异常边界。".repeat(
          8
        )}`
      )
    )
...
describe("engine compress", () => {
  ...
  test("keeps one completed and the last turn and evicts exactly the older ones", async () => {
    const { engine } = await freshEngine()
    ...
    expect(result.summary).toContain("[u0]")
  })
  ...
  test("reports source, evicted, retained, replacement and final token estimates", async () => {
    ...
    expect(result.finalTokensEst).toBe(result.retainedTokensEst + result.replacementTokensEst)
...
describe("engine retrieve", () => {
  ...
  test("by-history accepts the maximum page size of 50", async () => {
    ...
    expect(
      result.items.reduce((n, item) => n + Buffer.byteLength(item.content), 0)
  ...
  test("by-query returns namespace-scoped bm25-ordered hits", async () => {
    ...
    expect(otherSession).toMatchObject({ hits: [] })
  })
...
describe("rebuild consistency (mandatory)", () => {
  test("deleting index.db then reopening serves identical answers", async () => {
    ...
    expect(byHash).toMatchObject({ found: true, content: expect.any(String) })
  })
