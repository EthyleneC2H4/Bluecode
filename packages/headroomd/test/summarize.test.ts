  ...
  test("three segments: intent | action | tool anchors", () => {
    const turn = splitTurns([
      ...
      assistant("a4", "第四条不该出现（超过3条上限）。"),
      {
        ...
        parts: [{ type: "tool", tool: "bash", state: { status: "ok", output: "\nFAIL auth.test.ts\n" } }],
      },
    ])[0]!;
...
describe("historySummary", () => {
  test(">12 turns keeps first 2 + elision counter + last 8", () => {
    ...
    expect(summary).toContain("[T0]");
    expect(summary).toContain("[T1]");
    expect(summary).toContain("[T14]");
...
describe("messageSummary / messageExcerpt / keywords", () => {
  ...
  test("excerpt truncates at 200 chars", () => {
