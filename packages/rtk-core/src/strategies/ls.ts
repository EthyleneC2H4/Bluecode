...
export const lsStrategy: Strategy = ({ text }) => {
  ...
  const isLsLa =
    nonEmpty.length > 0 &&
    nonEmpty.filter(
      (line) => LS_PERM_RE.test(line) || LS_TOTAL_RE.test(line) || LS_SECTION_RE.test(line)
    ).length /
      nonEmpty.length >=
      0.9
  ...
  if (isLsLa) {
    ...
    for (const [index, line] of lines.entries()) {
      ...
      if (LS_TOTAL_RE.test(line)) {
        out.push({ text: line, sourceLine: index + 1, anchor: true, priority: PRI.critical })
        continue
      }
      ...
      if (nth <= KEEP_FILES_PER_DIR) {
        ...
        foldedFiles++
        bumpFold(fold, out, currentDir, (n) => `[+${n} file${n === 1 ? "" : "s"} in ${currentDir}]`)
      }
    }
  ...
  return fallbackStrategy({ text, toolId: "ls" })
}
