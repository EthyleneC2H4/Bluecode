...
function loadBaseline(): unknown | null {
...
export function validateReportStructure(report: unknown, label = "report"): Violation[] {
  ...
  if (typeof groups !== "object" || groups === null || !Array.isArray(rows)) {
    return [structuralViolation("report.shape", label, "groups object and perFixture array required")];
  }
  ...
  for (const group of GROUPS) {
    ...
    if (numeric.some((value) => typeof value !== "number" || !Number.isFinite(value))) {
      violations.push(structuralViolation("report.shape", `${label}:${group}`, "required metrics must be finite numbers"));
      continue;
    }
    const mustTotal = metrics.contextRecall?.mustHit?.total;
    const niceTotal = metrics.contextRecall?.niceToHave?.total;
    if (!Number.isInteger(mustTotal) || mustTotal < 0 || !Number.isInteger(niceTotal) || niceTotal < 0) {
      violations.push(structuralViolation("report.recallTotals", `${label}:${group}`, "context recall totals must be non-negative integers"));
    } else {
  ...
  for (const row of rows as Array<Record<string, unknown>>) {
    ...
    if (!GROUPS.includes(group as typeof GROUPS[number]) || typeof fixture !== "string") {
      violations.push(structuralViolation("report.fixtureRows", label, "each row needs a valid group and fixture"));
      continue;
    }
    ...
    if (degradedReason !== null && !DEGRADED.has(String(degradedReason))) {
      violations.push(structuralViolation("report.degradedReason", `${label}:${group}:${fixture}`, "unknown degradedReason"));
    }
  }
  ...
  for (const group of GROUPS) {
    ...
    if (!exact) {
      violations.push(structuralViolation("report.fixtureRows", `${label}:${group}`, "every fixture must occur exactly once in every group"));
    }
  }
  return violations;
}
...
export function checkBaseline(updateBaseline = false, opts: { skipLatency?: boolean } = {}): { passed: boolean; violations: Violation[] } {
  ...
  const baselineUnknown = loadBaseline();
  if (!baselineUnknown) {
  ...
  const currentUnknown = readReport();
  const structural = [
