...
describe("runner: quick-mode smoke across all four groups", () => {
  ...
  test("A/B/C/D all complete; report structure valid; groups observe their own contracts", async () => {
    ...
    for (const r of result.perFixture) {
      if (r.degradedReason !== null) {
        expect(["spawn_failed", "timeout", "crash", "protocol", "no_gain"]).toContain(
          r.degradedReason
        )
      }
    }
    ...
    const dInput = observations.find(
      (event) =>
        event.type === "headroom-input" && event.group === "D" && event.fixture === "long-session"
    )
    ...
    for (const group of ["C", "D"] as const) {
      const final = observations.find(
        (event) =>
          event.type === "final-context" &&
          event.group === group &&
          event.fixture === "long-session"
      )
      ...
      for (const retainedId of ["msg-52", "msg-53", "msg-54"]) {
        expect(final.messages.some((message) => message.info.id.endsWith(`:${retainedId}`))).toBe(
          true
        )
      }
  ...
  test("group B traffic really goes through rtk IPC (not the trivial passthrough path)", async () => {
    ...
    const records = result.perFixture.filter((r) => r.group === "B")
    expect(records.reduce((n, r) => n + r.replay!.runtime.rtkCalls, 0)).toBeGreaterThan(0)
    expect(records.reduce((n, r) => n + r.archiveRecoveryFound, 0)).toBeGreaterThan(0)
    const ls = records.find((r) => r.fixture === "tool-ls-large")!
    expect(ls.outTokens).toBeLessThan(ls.rawTokens)
  }, 120_000)
