 ...
 * the existing entry's canonical bytes).
 */
...
export async function writeObject(
  ...
  // Verify warm dedup before creating a temp entry: replay performs no writes.
  try {
    const existing = await readFile(path)
    if (!Buffer.from(existing).equals(Buffer.from(bytes))) {
      throw new Error(`cas: storage corruption at ${path}: canonical bytes differ (E_INTERNAL)`)
    }
...
export async function readObject(dataDir: string, hash: string): Promise<Uint8Array | null> {
  ...
  if (bytes.byteLength !== info.size) {
    throw new Error(
      `cas: storage corruption at ${path}: file changed between stat and read (E_INTERNAL)`
    )
  }
 ...
 * temp+hardlink publication as {@link writeObject}; dedup via EEXIST with canonical byte equality.
 */
