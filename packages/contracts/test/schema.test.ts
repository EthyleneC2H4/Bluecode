...
const compressParamsAligned: AssertEqual<
  CompressParams,
  z.input<typeof compressParamsSchema>
> = true
...
describe("rtk protocol v3", () => {
  test("hello handshake", () => {
    expect(helloSchema.parse({ proto: 3, pid: 4242 })).toEqual({ proto: 3, pid: 4242 })
    expect(helloSchema.safeParse({ proto: 1, pid: 4242 }).success).toBe(false)
    expect(helloSchema.safeParse({ proto: 3, pid: -1 }).success).toBe(false)
    expect(PROTOCOL_VERSION).toBe(3)
  })

  test("compress params require a session and fill budgetTokens default 512", () => {
    const parsed = compressParamsSchema.parse({
      tool: "bash",
      output: "line\n",
      sessionId: "sess-1",
    })
    ...
    expect(
      compressParamsSchema.parse({ tool: "t", output: "o", sessionId: "sess-1", budgetTokens: 64 })
        .budgetTokens
    ).toBe(64)
    expect(
      compressParamsSchema.safeParse({
        tool: "t",
        output: "o",
        sessionId: "sess-1",
        budgetTokens: 0,
    ...
    expect(compressParamsSchema.safeParse({ tool: "t", output: "o", sessionId: "" }).success).toBe(
      false
    )
  })
  ...
  test("fetch result discriminated union", () => {
    expect(
      fetchResultSchema.parse({ found: true, content: "text", nextCursor: null, truncated: false })
    ).toEqual({
      found: true,
      content: "text",
      nextCursor: null,
  ...
  test("request envelope", () => {
    expect(requestSchema.parse({ v: 3, id: "r_1_ab", op: "ping", params: {} })).toEqual({
      v: 3,
      id: "r_1_ab",
      op: "ping",
      params: {},
    ...
    expect(requestSchema.safeParse({ v: 3, id: "", op: "ping", params: {} }).success).toBe(false)
    expect(requestSchema.safeParse({ v: 3, id: "r", op: "nope", params: {} }).success).toBe(false)
  })

  test("response envelope both branches", () => {
    expect(responseSchema.parse({ v: 3, id: "r", ok: true, result: { pong: true } })).toEqual({
      v: 3,
      id: "r",
      ok: true,
      result: { pong: true },
    ...
    ).toEqual({ v: 3, id: "r", ok: false, error: { code: "E_UNKNOWN_OP", message: "?" } })
    // ok:false without error payload is invalid
    expect(responseSchema.safeParse({ v: 3, id: "r", ok: false }).success).toBe(false)
  })
})
...
describe("headroomd protocol v3", () => {
  ...
  test("compress result invariant via refine", () => {
    const compacted = {
      ...
      sourceDigests: [SHA.slice("sha256:".length)],
      memory: [],
      rawTokens: 1000,
      summaryTokens: 100,
    ...
    for (const patch of [
      ...
      { memory: undefined },
      { sourceDigests: [] },
      { sourceDigests: ["f".repeat(64)] },
      { replacedMessageIds: ["m1", "m2"] },
      { historyHash: "not-a-hash" },
    ]) {
      expect(headroomCompressResultSchema.safeParse({ ...compacted, ...patch }).success).toBe(false)
    }
    ...
    expect(headroomCompressResultSchema.safeParse({ ...compacted, compacted: false }).success).toBe(
      false
    )
  })
  ...
  test("retrieve hash mode", () => {
    ...
    expect(retrieveByHashParamsSchema.safeParse({ namespace: ns, hash: "zz" }).success).toBe(false)
    ...
    expect(retrieveByHashResultSchema.safeParse({ found: true, content: "raw text" }).success).toBe(
      true
    )
  ...
  test("retrieve query mode fills limit default 5", () => {
    ...
    expect(retrieveByQueryParamsSchema.parse({ namespace: ns, query: "q", limit: 2 }).limit).toBe(2)
  })
