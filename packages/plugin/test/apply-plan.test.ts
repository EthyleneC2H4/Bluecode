...
describe("applyPlanInPlace", () => {
  ...
  test("idempotent: re-applying when already replaced (structural archive detected)", () => {
    // Manually insert a compaction replacement message
    messages = [
      {
        ...makeMessage(
          `compaction-${"a".repeat(64)}`,
          "user",
          `${COMPACTION_MARKER} Already compacted`
        ),
        archive: { historyHash: "a".repeat(64), memory: [] },
      },
      makeMessage("msg-3", "user", "How are you?"),
    ]
    ...
    const plan = {
      refs: [],
      summary: "Summary",
      replacedMessageIds: [`compaction-${"a".repeat(64)}`],
      historyHash: "history-hash-123",
    }
