...
describe("config: parseOptions", () => {
  test("default values applied when input is empty", () => {
    ...
    expect(options.dataDir).toContain("bluecode")
...
test("different plugin options never share a cached sidecar entry", () => {
  clearSidecarCache()
  expect(resolveRtkEntry(parseOptions({ rtk: { entry: "/first/rtk" } }))).toBe("/first/rtk")
  expect(resolveRtkEntry(parseOptions({ rtk: { entry: "/second/rtk" } }))).toBe("/second/rtk")
  expect(resolveHeadroomEntry(parseOptions({ headroom: { entry: "/first/headroom" } }))).toBe(
    "/first/headroom"
  )
  expect(resolveHeadroomEntry(parseOptions({ headroom: { entry: "/second/headroom" } }))).toBe(
    "/second/headroom"
  )
})
