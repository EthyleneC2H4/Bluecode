...
export function startServer(io: ServerIo, options: ServeOptions = {}): RtkServerHandle {
  ...
  const engine: RtkEngine = createEngine({
    ...
    maxStorageBytes:
      options.maxStorageBytes ?? Number(process.env.BLUECODE_MAX_STORAGE_BYTES ?? 1024 ** 3),
  })
  ...
  async function handleLine(line: string): Promise<void> {
    ...
    if (!env.success) {
      await emitError(UNKNOWN_ID, {
        code: ErrorCode.E_PROTOCOL,
        message: "malformed request envelope (expected {v:3,id,op,params})",
        detail: { line: lineNo },
      })
      return
    }
