...
合并后完整复验出现472通过/1失败：客户端在绝对握手期限到达后正常断开，测试server的滴灌timer仍可能写入，未监听的EPIPE被测试框架判为失败。修复测试fixture对预期EPIPE/ECONNRESET的处理，并等待server关闭完成；其他错误继续抛出，原超时及1400ms上界断言保持不变。该场景连续10次复验通过，随后重新执行完整verify。此修复只涉及测试生命周期，不修改生产握手期限。

## 2026-09-05 · RTK / headroomd 可靠工程版

本条对应[六阶段实施计划](superpowers/plans/2026-09-05-reliability.md)和[完整验收记录](reliability-implementation.md)，后文早期一次性评测与接口描述按历史记录保留，当前行为以本次架构/协议文档为准。

1. **视图只能应用一次、旧内容校验不足。** 宿主每次重建原始数组，内存里的pending plan在一次splice后消费，不能持续节省。改为持久活动view，逐次按真实namespace、epoch、完整有序内容digest验证重放；宿主源编辑/删除事件还会取消在途generation，不能仅拿旧view和旧snapshot互相校验。
2. **压缩目标压过内容保护。** RTK失败块中的checkmark、combined diff以及来源行范围需要保守处理。现在把预算定义为软目标，保留代码、变更和完整失败证据，显式报告超额与无收益；headroom用户要求逐字保存，完整tool input/output/error进入预算与证据记忆。
3. **存储协议跨进程不等于隔离和恢复。** RTK在独占发布锁内先持久预留，再CAS发布和grant，失败/崩溃预留可安全回收；headroom锁绑定根目录，活动view保存完整发布快照，FTS分片inventory支持部分丢失重建。离线迁移只改副本，marker严格核对来源及项目映射。
4. **真实工厂与运行时回放不同。** 去除全局env后，factory漏传headroom dataDir，mock connect测试无法发现。新增无环境变量的真实子进程冷启动，显式传入根目录；RTK失败create也必须清理无人接管的重启timer，并有界终止忽略SIGTERM的半启动进程。宿主ignored/error/compacted/interrupted字段需要保护，不能把隐藏旧正文重新变成摘要。
5. **测量定义必须可审查。** A/B/C/D改为真实runtime多步回放，独立自然问题和确定性内容检查；累计重复上下文、问题、检索页返回。门禁重算行级观测与汇总，查询命中的第5条续页也不能漏掉。主场景净输入减少25.25%，主动全文展开仅6.95%且门禁失败；不能用最终上下文比例代替账单节省。
6. **分帧异常也会受chunk边界影响。** 全仓测试发现有效响应与后续超大帧共处一read时，push抛错会丢掉已收集的完整前缀。FrameOverflowError现在携带completedLines，客户端先处理有效响应再处理溢出；新增每种分块位置的确定性回归。首轮471通过/1失败的证据没有被归咎于测试抖动。

上述改动经过分域及独立最终审查；最后命令结果、平台范围与基线见验收记录。所有实测来自本地合成回放，不构成企业线上效果或个人私有源码贡献。
