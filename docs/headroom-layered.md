...
0～4 轮对照后选择一轮：相对四轮组合累计输入进一步降低 8.30%，可见事实不退步；零轮额外移出三条事实，详见[轮数实验](headroom-retention.md)。`legacy` 默认仍保留四轮。已有配置显式填写 4 时仍使用四轮，需要修改或省略该字段才能采用 layered 的新默认值。

恢复持久活动视图时按当前策略检查兼容性，旧的无策略标记计划视为 `legacy`。策略不兼容时清除活动视图指针后重建，原文与节点继续保留。每次重放还按当前保留轮数检查操作覆盖范围；旧视图触碰当前受保护尾部时清理活动指针并重新规划，原文保留。已增强视图缺少完整的发布服务配置证明，因此重启时也重新按当前配置准备规则视图；可能增加一次准备工作，避免关闭或更换摘要服务后继续无条件复用旧增强。
...
较早且完成的工具结果会被原文确认后替换为来源标记和简短观察记录。用户需求、修正和含义不明确的代码保持原文；只有明确标注为材料的文本范围可以外置。默认保护最近一个完整轮次与当前活动轮次；显式 `retainRecentTurns` 可调整该数量，包括仅保留当前轮次的 0。相同工具参数但输出不同的结果保留版本区分；失败诊断不能仅按年龄或一条无关“通过”记录消失。
