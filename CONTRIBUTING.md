...
2. **Eval gate green if you touched rtk / headroomd / eval.** Run `bun run eval`
   ...
   compression-regression (>2pp worse), recall regression, or a p95-latency
   blowup. If your change *intentionally* moves numbers, re-run with
   `--update-baseline`, paste the old/new table into the PR, and say why.
3. **Respect the dependency direction** (CI does not enforce this yet — humans must):
   `contracts ← shared ← {rtk-core, headroomd} ← rtk ← plugin`. Sidecar packages
   may only import from `@bluecode/contracts` and `@bluecode/shared`.
